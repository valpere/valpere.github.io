...
Session ends (exit / /exit)
  └─ Stop hook: .claude/hooks/session-end.sh
       ...
       ├─ Try agy -p            (Gemini 3.5 Flash → 3.1 Pro, cheapest first)
       ├─ Try opencode run --format json  (ollama/*:cloud models)
...
the sole session-history mechanism here (per-project-local, git-ignored).

The skill works standalone (without hooks) — you just run `/session-end`
manually. The hooks add automation.

**Log file**: `.claude/session-log.md` — gitignored (see `.gitignore`).

**Troubleshooting**:
```bash
tail -30 ~/.cache/valpere.github.io/hooks.log
